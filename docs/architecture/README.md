...
>
> - **Containers (12, hand-written TS, Node-validated):** mp4·mov, webm·mkv, ogg, wav, aiff, caf, mp3, flac, adts, mpegts (ts·m2ts·mts), avi, hls (playlist). Probe + demux across all; **muxers** for mp4/webm/ogg/mpegts/flac/mp3/adts (chunk-seam; FLAC via the pure-TS `flac-encode` codec driver, ADR-086), WAV raw-PCM packets (ADR-116), and AVI RIFF packet layout (ADR-117); WAV/AIFF/CAF PCM conversion still uses `transformPcm`.
> - **Pure-TS ops (Node-validated):** `probe`, `demux`, `remux` + keyframe `trim` (stream-copy), `decrypt` (`cenc`/`cens`/`cbcs`/`hls-aes128`/`hls-sample-aes`), PCM `convert` (format/gain/fade/BS.775-mix/**resample**/dynamics/biquad, plus direct interleaved WAV s16 sample-rate-only resample), **FLAC decode** (STREAMINFO-MD5 bit-exact).
> - **WebCodecs/GPU codec tier (browser-validated):** `decode`/`encode`/`convert` re-encode/`seek` (hardware-first, close-race-safe); **ADTS AAC→WAV PCM extraction** via native `AudioDecoder` with `wasm-aac` fallback plus direct wasm-to-WAV-s16 for small no-DSP jobs; **GPU video filters** (WebGPU + Canvas2D) for geometry + colorspace/tonemap, a **pure-TS CPU video filter** floor for no-GPU browsers, and an **`AudioData` audio filter** (resample/remix/gain).
