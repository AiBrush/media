...
> - **Pure-TS ops (Node-validated):** `probe`, `demux`, `remux` + keyframe `trim` (stream-copy), `decrypt` (`cenc`/`cens`/`cbcs`/`hls-aes128`/`hls-sample-aes`), PCM `convert` (format/gain/fade/BS.775-mix/**resample**/dynamics/biquad), **FLAC decode** (STREAMINFO-MD5 bit-exact).
...
> - **Streaming output building blocks:** fragmented/CMAF MP4, WebM/MKV live Cluster output, true WebM/MKV Cluster-on-write remux, and the `StreamTarget` sink are wired into the public sink/remux/mux surface; MP4/MOV remux and keyframe trim both have source-range lazy materialization paths for large inputs.
