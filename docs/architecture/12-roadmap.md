...
- **More codecs:** VP9, AV1, HEVC (WebCodecs where present), with only the **registered, license-compatible WASM tails** loaded lazily on a supported miss (ADR-005/225); the shipped tails are decode-only for VP8/VP9, AV1, AAC, and MP3, with the documented Opus/Vorbis encode paths. **FLAC decode is pure TS, not WASM** (ADR-024): it is lossless/integer, so the TS decoder is bit-exact (validated via STREAMINFO-MD5 on the IETF FLAC conformance corpus) and ~kilobytes — no toolchain needed. Per ADR-025, the WebCodecs/WASM-tier codecs are validated on the browser/target runtime, the pure-TS tier in Node CI.
...
- **decrypt:** CENC (`cenc`/`cens`/`cbcs`), HLS full-segment AES-128, and HLS TS SAMPLE-AES via WebCrypto + TS box parse.
...
- **Fluent chain (ADR-010):** shipped as additive sugar over the flat ops; the serialized job runner stays
  the worker-boundary direction for later executor consolidation.
