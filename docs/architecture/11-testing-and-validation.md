...
6. **Performance** — multi-sample (`n>1`) wall/throughput/peakMemory/longtasks, re-measured fresh. **The benchmark's perf margins were cached single-sample [data: Finding 7]; we do not trust or reuse them — we re-measure.** The concrete harnesses (warmup + median + separate RSS pass + checksum sink + a `--check` regression gate) and the committed pure-TS baseline numbers are in [`14-benchmarks.md`](14-benchmarks.md); the WebCodecs/GPU tier is measured on the browser/target runtime (ADR-025).
   The WAV raw-PCM stream proof additionally runs `bun run bench-session12-wav-pcm-stream` on the exact real s24 corpus: first-chunk range bytes, complete frame count, deterministic sampled PCM checksum, and fresh warmup-plus-multi-sample timings (ADR-226).
...
The sibling `aibrush.lib/media-test` suite is a **versioned external regression battery** (13 families; the original architecture evidence had 558 features and the Session 12 suite has 563). We register `aibrush-media` as an engine and must run every cell in the current suite, record its version/count, (a) win in aggregate, and (b) pass on the **strict** oracles, with our own additional anti-cheat gates from §5. It is the acceptance test for "did we actually build the best-of-the-best."
